...
pub fn ul(coefficients: Vec<Vec<f64>>, constants: Vec<f64>) -> Option<Vec<f64>> {
    ...
    for i in 0..n {
        for j in i..n {
            ...
            sum = 0.0;
            for k in 0..i { // tu jest jeszcze jakiś błąd do poprawy
                sum += l[j][k] * u[k][i];
            }
...
mod tests {
    ...
    fn test_sole_ul0() {
        ...
        let res = ul(coefficients, constants);
        assert!(res.is_some());
